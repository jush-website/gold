    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            if (err.code === 'auth/unauthorized-domain') {
                setError('目前預覽網址未被授權 (Firebase 安全限制)。請使用下方「訪客試用」登入測試功能。');
    ...
    return (
        <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 p-8 rounded-3xl shadow-2xl text-center">
                ...
                {error && (
                    <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-xl mb-4 text-xs text-left flex flex-col gap-1">
                        <div className="flex items-center gap-2 font-bold"><AlertCircle size={14}/> 登入發生錯誤</div>
                        <div>{error}</div>
                    </div>
                )}
                ...
                <div className="space-y-3">
                    ...
                    {/* Guest Login fallback for preview environments */}
                    <button 
                        onClick={handleGuestLogin}
                        disabled={loading}
                        className="w-full bg-gray-700/50 hover:bg-gray-700 text-gray-300 font-bold py-3 px-6 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2 border border-gray-600"
                    >
                         <User size={18} />
                         訪客試用 (匿名登入)
                    </button>
                    {showGuestLogin && <p className="text-[10px] text-gray-500">提示：正式部署後請至 Firebase Console 將網域加入授權清單，即可使用 Google 登入。</p>}
                </div>
...
export default function App() {
    ...
    useEffect(() => {
        // Force inject Tailwind Script if not present
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400">Welcome, {user.isAnonymous ? 'Guest' : user.displayName}</p>
                            </div>
                        </div>
