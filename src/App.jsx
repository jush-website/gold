                 ...
                 {currentView === 'debt' && (
                    <div className="h-full flex flex-col animate-[fadeIn_0.3s]">
                        <div className="px-4 pt-4 pb-2 shrink-0 space-y-3 z-10">
                            ...
                            {/* 借款頁籤：未結清 vs 已結清 */}
                            <div className="flex bg-white rounded-2xl p-1.5 shadow-sm border border-gray-100 mt-2">
                                <button onClick={() => setDebtTab('active')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${debtTab === 'active' ? 'bg-rose-50 text-rose-600 shadow-sm border border-rose-100' : 'text-gray-400 hover:bg-gray-50'}`}>未結清 ({activeDebtsList.length})</button>
                                <button onClick={() => setDebtTab('settled')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${debtTab === 'settled' ? 'bg-emerald-50 text-emerald-600 shadow-sm border border-emerald-100' : 'text-gray-400 hover:bg-gray-50'}`}>已結清 ({settledDebtsList.length})</button>
                            </div>
                        </div>
                        ...
                        <div id="debt-scroll-container" className="flex-1 overflow-y-auto hide-scrollbar px-4 pb-24 space-y-4 pt-1">
                            {displayDebts.length === 0 ? (
                                <div className="text-center py-10 flex flex-col items-center">
                                    ...
                                    <div className="text-gray-400 font-bold">{debtTab === 'active' ? '目前無待還借款' : '目前無已結清紀錄'}</div>
                                </div>
                            ) : displayDebts.map(debt => {
                                const { repaid, remaining, isSettled } = debt;
